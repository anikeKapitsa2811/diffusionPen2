...
class WordLineDataset1(Dataset):
    #
    # TODO list:
    #
    #   Create method that will print data statistics (min/max pixel value, num of channels, etc.)   
...
class WordLineDataset(Dataset):
    ...
    def __finalize__(self):
        ...
        #save_path = './IAM_dataset_PIL_style'
        save_path = '/cluster/datastore/aniketag/allData/diffPen//saved_GNHK_data/'
        ...
        self.wclasses = len(writer_ids)
        print('Number of writers', self.wclasses," self.wclasses:",self.wclasses," writer_ids:",writer_ids[:5])
        if self.character_classes is None:
            res = set()
             #compute character classes given input transcriptions
            for _,transcr,_,_ in tqdm(data):
    ...
    def __getitem__(self, index):
        ...
        if len(positive_samples) >= 5:
            ...
            random_samples_ = random.choices(positive_samples, k=5)

            #random_samples_ = random.sample(positive_samples_, k=len(positive_samples_))
            # Retrieve the corresponding images
            style_images = [i[0] for i in random_samples_]
    ...
    def collate_fn(self, batch):
        ...
        images_batch = torch.stack([b.clone() for b in img])
        char_tokens_batch = torch.stack([b.clone() for b in char_tokens])
        images_pos = torch.stack([b.clone() for b in positive])
        images_neg = torch.stack([b.clone() for b in negative])
        s_imgs = torch.stack([b.clone() for b in s_imgs])
        pixel_values_img = torch.stack([b.clone() for b in pixel_values_img])
        pixel_values_pos = torch.stack([b.clone() for b in pixel_values_pos])
        pixel_values_neg = torch.stack([b.clone() for b in pixel_values_neg])
...
class GnhkDataset_style(WordLineDataset):
    def __init__(self, basefolder, subset, segmentation_level, fixed_size, transforms):
        super().__init__(basefolder, subset, segmentation_level, fixed_size, transforms)
        self.setname = 'GNHK'
        self.trainset_file = '{}/{}/set_split/trainset.txt'.format(self.basefolder, self.setname)
...
class CVLDataset_style(WordLineDataset):
    ...
    def main_loader(self, subset, segmentation_level) -> list:
        ...
        def gather_iam_info1(self, set='train', level='word'):
            print("2.here", subset, level)
            if subset == 'train':
                #valid_set = np.loadtxt(self.trainset_file, dtype=str)
                valid_set = np.loadtxt('./utils/aachen_iam_split/train_val.uttlist', dtype=str)
                #print(valid_set)
...
class Mixed_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # Classify
        logits,_ = self.classifier(pooled_features)
...
def performance(pred, label):
    ...
    #print("\n pred shape", type(pred)," pred.shape:",pred)
    #print("\n label shape", type(label)," label.shape:",label)
    
    if 0:
        print(f"pred type: {type(pred)}")
        print(f"pred shape: {pred.shape if torch.is_tensor(pred) else 'not a tensor'}")
        print(f"label type: {type(label)}")
        print(f"label shape: {label.shape if torch.is_tensor(label) else 'not a tensor'}")

        # also helpful:
        print(f"Sample pred values: {pred[0][:5].detach().cpu().numpy()}")   # first row, first 5 classes
        print(f"Sample label values: {label[0] if label.ndim > 0 else label}")
...
def train_class_epoch(model, training_data, optimizer, args):
    ...
    for i, data in enumerate(pbar):
        ...
        if args.dataset == 'CVL' or args.dataset == 'GNHK':
...
def val_epoch_triplet(val_loader, model, criterion, optimizer, device, args):
    ...
    for i, data in enumerate(pbar):
        ...
        # print type and shape of all above var 
        print(f"Type of anchor_out: {type(anchor_out)}, Shape: {anchor_out.shape if torch.is_tensor(anchor_out) else 'not a tensor'}")
        print(f"Type of positive_out: {type(positive_out)}, Shape: {positive_out.shape if torch.is_tensor(positive_out) else 'not a tensor'}")
        print(f"Type of negative_out: {type(negative_out)}, Shape: {negative_out.shape if torch.is_tensor(negative_out) else 'not a tensor'}")
...
def train_epoch_mixed(train_loader, model, criterion_triplet, criterion_classification, optimizer, device, args):
    ...
    pbar = tqdm(train_loader)
    for i, data in enumerate(pbar):
        #   img, transcr, char_tokens_batch, wid, images_pos, images_neg, cla, s_imgs, img_path, pixel_values_img, pixel_values_pos, pixel_values_neg
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='GNHK', help='dataset name')
    parser.add_argument('--batch_size', type=int, default=640, help='input batch size for training')
    ...
    parser.add_argument('--save_path', type=str, default=baseModelDir+'/style_models', help='path to save models')
    ...
    parser.add_argument('--mode', type=str, default='triplet', help='mixed for DiffusionPen, triplet for DiffusionPen-triplet, or classification for DiffusionPen-triplet')
    #parser.add_argument('--mode', type=str, default='classification', help='mixed for DiffusionPen, triplet for DiffusionPen-triplet, or  for DiffusionPen-triplet')
    ...
    if args.dataset == "CVL":
        ...
        validation_size = int(0.0001 * len(train_data))
        ...
        # 1st example o ftran analyse 
        #   img, transcr, char_tokens_batch, wid, images_pos, images_neg, cla, s_imgs, img_path, pixel_values_img, pixel_values_pos, pixel_values_neg

        example = train_data[0]
        print('len example', len(example))
        print('type example[0]', type(example[0]))
        print('example[0] shape', example[0].shape)
        print('example[1]', example[1])
        print('example[2] shape', example[2].shape)
        ...
        style_classes = 283#1139
    
    elif args.dataset == "GNHK":
    
        myDataset = GnhkDataset_style
        dataset_folder = gnhkPath
        aug_transforms = [lambda x: affine_transformation(x, s=.1)]
