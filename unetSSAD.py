...
logger.info('--- GnhkOneDiff training ---')
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.B, self.L = 2, 40        # batch size, sequence length
        self.n_crops = 5
        self.d_s, self.d_x = 1280, 320

        self.ContentEncoder = ContentEncoder(d_s=self.d_s, d_x=self.d_x, seq_len=self.L, mode="TS", n_crops=self.n_crops)
    ...
    def forward(self, x, timesteps=None, context=None, y=None, mix_rate=None, style_extractor=None, **kwargs):
        ...
        #print('\n x', x.shape, x.dtype)
        #logger.info(f'\n x: {x.shape}, {x.dtype}')
        #print('\n timesteps', timesteps.shape, timesteps.dtype)
        #logger.info(f'\n timesteps: {timesteps.shape}, {timesteps.dtype}')
        
        """ 
            x torch.Size([2, 4, 8, 32]) torch.float32

            timesteps torch.Size([2]) torch.int64

            y torch.Size([2]) torch.int64
        ...
        if context is not None:
            ...
            try:
                ...
                context keys: dict_keys(['input_ids', 'token_type_ids', 'attention_mask'])
                input_ids: torch.Size([2, 40])
                token_type_ids: torch.Size([2, 40])
                attention_mask: torch.Size([2, 40])
            
            except:
                
                print('\n context', type(context))
                #logger.info(f'\n context: dict', type(context))
        """ 
        
        hs = []
        
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)#.to(x.device)
        #print("0.init t_emb:", t_emb.shape, t_emb.dtype)
        emb = self.time_embed(t_emb)
        #print("1.init emb:", emb.shape, emb.dtype)

        """ 
        0.init t_emb: torch.Size([BS, 320]) torch.float32
        1.init emb: torch.Size([BS, 1280]) torch.float32
        ...
        if context is not None:
            ...
            #print('\n 2.context:', context.shape)
            #  2.context: torch.Size([2, 40, 768])
  
            #print('\n 2.context:', context)         
            if self.cont_dim == 320:
                context = self.text_lin(context)#.unsqueeze(1)

        #print('\n 222.context:', context.shape," style_extractor.shape:",style_extractor.shape," t_emb.shape:",t_emb.shape)
        #222.context: torch.Size([2, 40, 320])  style_extractor.shape: torch.Size([10, 1280])  t_emb.shape: torch.Size([2, 320])

        if 1:
            context,emb = self.ContentEncoder(context,style_extractor,emb)  #torch.Size([2, 40, 1280])
            #print('\n 3.context:', context.shape," emb.shape:",emb.shape," style_extractor.shape:",style_extractor.shape)

        #print("0.final t_emb:", t_emb.shape, t_emb.dtype)
        #logger.info(f'0.final t_emb: {t_emb.shape}, {t_emb.dtype}')


        
        #print('1.final context:', context.shape, context.dtype)
        #logger.info(f'1.final context: {context.shape}, {context.dtype}')
        #print("1.final t_emb:", t_emb.shape, t_emb.dtype)
        #logger.info(f'1.final t_emb: {t_emb.shape}, {t_emb.dtype}')
        ...
        for module in self.input_blocks:
            #print('\n\t before input block:', h.shape," emb.shape:",emb.shape, "\t context.shape:",context.shape)
            # no SSAD	 before input block: torch.Size([2, 320, 8, 32])  emb.shape: torch.Size([2, 1280]) 	 context.shape: torch.Size([2, 40, 320])
            # SSAD 	     before input block: torch.Size([2, 4, 8, 32])  emb.shape: torch.Size([2, 320]) 	 context.shape: torch.Size([2, 40, 320])
